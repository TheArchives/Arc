...
class LinePlugin(ProtocolPlugin):
    ...
    def commandLbook(self, data):
        "/lbook pagenumber - Guest\nExplains lsystem in book form."
        if len(parts) > 1:
            if parts[1].lower() == "1":
                data["client"].sendServerMessage("lbook [Page 1 of 12]")
                ...
                data["client"].sendServerMessage("lbook [Page 2 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 3 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 4 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 5 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 6 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 7 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 8 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 9 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 10 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 11 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("lbook [Page 12 of 12]")
                data["client"].sendSplitServerMessage(
                ...
                data["client"].sendServerMessage("There's no page for that.")
        else:
            data["client"].sendServerMessage("You forgot to specify a page number.")

    @config("category", "build")
    @config("rank", "op")
    def commandRec_Axiom(self, data):
        ...
        if len(parts) != 2:
            ...
            if parts[0] == '//rec_axiom':
                try:
                    data["client"].var_axiom = data["client"].var_axiom + parts[1]
                except:
                    data["client"].sendServerMessage("You have not started recording an axiom yet")
                    return
            else:
                data["client"].var_axiom = parts[1]
            var_divisions64 = int(len(data["client"].var_axiom) / 64)
            data["client"].sendServerMessage("The axiom has been recorded as:")
            for i in range(var_divisions64):
                data["client"].sendServerMessage(data["client"].var_axiom[i * 64:(i + 1) * 64])
            data["client"].sendServerMessage(data["client"].var_axiom[var_divisions64 * 64:])

    @config("category", "build")
    @config("rank", "op")
    def commandRec_Production(self, data):
        "/rec_production item production - Op\nRecords production recipes for lsystems."
        if len(parts) != 3:
            if len(parts) == 2:
                if parts[1] == 'reset':
                    self.var_production = {}
                    data["client"].sendServerMessage("Your production recipes have been cleared")
                    return
            data["client"].sendServerMessage("Please enter an item and a production recipe for it to be")
            data["client"].sendServerMessage("replaced with (type in 'reset' to reset all the recipes).")
            data["client"].sendServerMessage("Use //rec_production to add additional recipe choices for")
            data["client"].sendServerMessage("the item.")
        else:
            keyitem = parts[1]
            ...
            for item in parts[2]:
                if item != 'F' and item != 'B' and item != '[' and item != ']' and item != '+' and item != '-' and item != '<' and item != '>' and item != 'G' and item != 'V' and item != 'N' and item != 'M' and item != '{' and item != '}' and item != 'A' and not item.isdigit() and item != 'T' and item != ',':
                    ...
                    data["client"].sendServerMessage("(use commas between phrases to have it select randomly from them)")
                    return
            ...
            data["client"].sendServerMessage("The production recipe has been recorded as:")
            item_list = []
            for key in self.var_production:
                item_list.append(key + " : " + str(self.var_production[key]))
            data["client"].sendServerList(item_list)

    @config("category", "build")
    @config("rank", "op")
    def commandLsystem(self, data):
        "/lsystem blocktype standarddistance\nzxrotation yzrotation xyrotation level [x y z x2 y2 z2] - Op\nDraws an lsystem."
        try:
            axiom = data["client"].var_axiom
            production = self.var_production
        except:
            data["client"].sendServerMessage("Please use /rec_axiom first (and /rec_production if you")
            data["client"].sendServerMessage("want to make a fractal.")
            return
        if len(parts) < 13 and len(parts) != 7:
            data["client"].sendServerMessage("Please enter a blocktype, standarddistance(which is how far")
            data["client"].sendServerMessage("the drawer will go per F, B, G, or V), the rotations are how")
            data["client"].sendServerMessage("many degrees the drawer will rotate for each +,-,>,<,{,} along")
            data["client"].sendServerMessage("it's respective axis and finally the level")
            data["client"].sendServerMessage("(an integer between 0 and 5) which determines")
            data["client"].sendServerMessage("how many times the characters in the production will be")
            data["client"].sendServerMessage("replaced with their respective productionrecipe phrases.")
            data["client"].sendServerMessage("Note that you must place two blocks.")
            data["client"].sendServerMessage("The first block you place will determine where the drawer starts")
            data["client"].sendServerMessage("and the second will determine where it initially points toward.")
        else:
            # Try getting the standarddistance
            try:
                standarddistance = float(parts[2])
            except:
                data["client"].sendServerMessage("The standarddistance must be a real number")
                return
                # Try getting the zxrotation
            try:
                zxrotation = int(parts[3])
            except:
                data["client"].sendServerMessage("The zxrotation must be an integer")
                return
            if zxrotation > 360 or zxrotation < 0:
                data["client"].sendServerMessage("The zxrotation must be between 0 and 360 degrees")
                return
                # Try getting the yzrotation
            try:
                yzrotation = int(parts[4])
            except:
                data["client"].sendServerMessage("The yzrotation must be an integer")
                return
            if yzrotation > 360 or yzrotation < 0:
                data["client"].sendServerMessage("The yzrotation must be between 0 and 360 degrees")
                return
                # Try getting the xzrotation
            try:
                xyrotation = int(parts[5])
            except:
                data["client"].sendServerMessage("The xyrotation must be an integer")
                return
            if xyrotation > 360 or xyrotation < 0:
                data["client"].sendServerMessage("The xyrotation must be between 0 and 360 degrees")
                return
                # Try getting the level
            try:
                level = int(parts[6])
            except:
                data["client"].sendServerMessage("The level must be an integer")
                return
            if level < 0 or level > 5:
                data["client"].sendServerMessage("The level must be between 0 and 5")
                return
            ...
            if limit != -1:
                ...
                if (num_movements * standarddistance > limit):
                    data["client"].sendServerMessage("Sorry, that area is too big for you to an lsystem on.")
                    data["client"].sendServerMessage("(Limit is %s)" % limit)
                    return
            world = data["client"].world

            def generate_changes():
                block = data["client"].getBlockValue(parts[1])
                ...
                for index in range(len(finalsequence)):
                    ...
                    if item == "F":
                        ...
                        for coordtuple in coordinatelist2:
                            i, j, k = coordtuple
                            try:
                                if not data["client"].allowedToBuild(i, j, k):
                                    data["client"].sendServerMessage("You do not have permision to build here.")
                                    return
                            except:
                                pass
                            var_override = True
                            try:
                                ...
                                pass
                            if not var_override:
                                data["client"].queueTask(TASK_BLOCKSET, (i, j, k, block), world=world)
                                data["client"].sendBlock(i, j, k, block)
                            yield
                    elif item == "A":
                        targetlocation = (drawer_location[0] + drawer_orientationvector[0] * standarddistance,
                                          drawer_location[1] + drawer_orientationvector[1] * standarddistance,
                        ...
                        try:
                            ...
                            data["client"].sendServerMessage("No saved location error ('[' but no ']')")
                            return
                    elif item == "N":
                        pass
                    elif item == "M":
                        ...
                        try:
                            ...
                            data["client"].sendServerMessage("T must be followed by a number from 0 to 49.")
                            return
            ...
            def do_step():
                ...
                try:
                    ...
                    if fromloc == "user":
                        data["client"].sendServerMessage("Your lsystem just completed.")
                    pass
