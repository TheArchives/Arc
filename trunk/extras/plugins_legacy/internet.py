...
class InternetPlugin(object):
    ...
    def gotClient(self):
        ...
        try:
            getattr(data["client"].factory, "twlog")
        except AttributeError:
            self.factory.twlog = open("logs/twitter.log", "a")
        self.url = ""

    @config("custom_cmdlog_msg", "just logged into Twitter.")
    @config("category", "info")
    @config("disabled-on", ["cmdblock", "irc"])
    def commandTlogin(self, data):
        "/tlog username password - Guest\nReplace username and password to login to Twitter."
        if len(parts) < 3:
            data["client"].sendServerMessage("Please input a username and password.")
        else:
            self.tuser = str(parts[1])
            self.tpass = str(parts[2])
            data["client"].sendServerMessage("Username: " + COLOUR_RED + self.tuser)
            data["client"].sendServerMessage("Password: " + COLOUR_RED + self.tpass)
            self.factory.twlog.write(
                self.tuser + "(" + data["client"].username + ")" + " has logged into twitter.\n")
            self.factory.twlog.flush()

    @config("category", "info")
    @config("disabled-on", ["cmdblock", "irc"])
    def commandTweet(self, data):
        "/tweet tweet - Guest\nSend a tweet to Twitter after using /tlog."
        if self.tuser == "":
            data["client"].sendServerMessage("Please do /tlog first.")
        else:
            ...
            data["client"].sendServerMessage("You have successfully tweeted.")
            self.factory.twlog.write(
                self.tuser + "(" + data["client"].username + ")" + " has tweeted: " + msg + "\n")
            self.factory.twlog.flush()

    @config("category", "info")
    @config("disabled-on", ["cmdblock", "irc"])
    def commandTDetails(self, data):
        "/tdetails - Guest\nGives you your Twitter login details, from /tlog."
        if self.tuser == "":
            data["client"].sendServerMessage("Username: " + COLOUR_RED + "Not entered!")
        else:
            data["client"].sendServerMessage("Username: " + COLOUR_RED + self.tuser)
        if self.tpass == "":
            data["client"].sendServerMessage("Password: " + COLOUR_RED + "Not entered!")
        self.twlog.write(self.tuser + "(" + data["client"].username + ")" + " has checked their Twitter details.\n")
        self.twlog.flush()

    @config("category", "build")
    @config("rank", "admin")
    def commandRec_url(self, data):
        "/rec_url URL - Builder\nRecords an url to later imagedraw it."
        if noimagedraw == True:
            data["client"].sendServerMessage("Imagedraw is currently disabled due to server problems.")
        else:
            if len(parts) == 1:
                ...
                if parts[0] == '//rec_url':
                    ...
                    self.url = (" ".join(parts[1:])).strip()
            var_divisions64 = int(len(data["client"].url) / 64)
            data["client"].sendServerMessage("The url has been recorded as:")
            for i in range(var_divisions64):
                data["client"].sendServerMessage(data["client"].url[i * 64:(i + 1) * 64])
            data["client"].sendServerMessage(data["client"].url[var_divisions64 * 64:])

    @config("category", "build")
    @config("rank", "admin")
    def commandImagedraw(self, data):
        ...
        if len(parts) < 8 and len(parts) != 3:
            data["client"].sendServerMessage("Please enter whether to flip or not (rotation)")
        ...
        try:
            ...
            data["client"].sendServerMessage("Rotation must be an integer.")
            return
            # Try to get flip
        flip = parts[1].lower()
        if flip not in ["true", "false"]:
            data["client"].sendServerMessage("Flip must be true or false.")
            return
            # Try to get url
        if self.url == "":
            data["client"].sendServerMessage("You have not recorded an url yet (use /rec_url).")
            return
        if self.url.find('http:') == -1:
            data["client"].sendServerMessage("Only the http protocol is supported.")
            return
        ...
        try:
            ...
            data["client"].sendServerMessage("The url of the image is invalid or unaccesible.")
            return
        if rotation != 0:
        ...
        def generate_changes():
            try:
                for i in range(x, x2 + 1):
                    for j in range(y, y2 + 1):
                        for k in range(z, z2 + 1):
                            if not data["client"].allowedToBuild(i, j, k) and not overriderank:
                                return
                            if flip == "true":
                                if orientation == 0:
                ...
                data["client"].sendServerMessage("Out of bounds imagedraw error.")
                return
        ...
        def do_step():
            ...
            try:
                ...
                if fromloc == "user":
                    data["client"].sendServerMessage("Your imagedraw just completed.")
                pass

        do_step()

serverPlugin = InternetPlugin