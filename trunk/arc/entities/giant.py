...
if var_cango:
    ...
    self.client.sendBlock(x, y+2, z, block)
    world[x, y+2, z] = block
    self.client.queueTask(TASK_BLOCKSET, (x, y+2, z, block), world=world)
    self.client.sendBlock(x, y+3, z, block)
    ...
    if closestdistance < 3:
        closestclient.sendServerMessage("FEE FI FOE FUM!")
    if closestdistance < 2:
        sx,sy,sz,sh = world.spawn
        closestclient.teleportTo(sx,sy,sz,sh)
        self.client.sendPlainWorldMessage("&7%s has been eaten by a giant." % closestclient.username)
    i,k = closestposition
