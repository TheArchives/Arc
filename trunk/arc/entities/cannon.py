        ...
        if entity[4] not in entities_childerenlist:
            if (cx, cy, cz) == (ax, ay, az) and time() - var_timeofchange < 2:
                if entity[6]:
                    ...
                    ix, iy, iz = int(round(rx * distancebetween + rx + px)), int(
                        round(ry * distancebetween + ry + py)), int(round(rz * distancebetween + rz + pz))
                    entitylist.append(["cannonball", (
                    rx * distancebetween + 2 * rx + px, ry * distancebetween + 2 * ry + py,
                    rz * distancebetween + 2 * rz + pz), 2, 2, [rx, ry, rz], i])
