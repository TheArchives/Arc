...
class BuildLibPlugin(ProtocolPlugin):
            ...
            if limit != -1:
                ...
                if (((radius * 2) ** 3) - (((radius - 1) * 2) ** 3) > limit) or limit == 0:
                    self.client.sendServerMessage(
                        "Sorry, that area is too big for you to hsphere (Limit is %s)" % limit)
                    return
                # Draw all the blocks on, I guess
            # We use a generator so we can slowly release the blocks
            # We also keep world as a local so they can't change worlds and affect the new one
            ...
            def generate_changes():
                try:
                    for i in range(-radius - 1, radius):
                        for j in range(-radius - 1, radius):
                            for k in range(-radius - 1, radius):
                                if (i ** 2 + j ** 2 + k ** 2) ** 0.5 < radius and (i ** 2 + j ** 2 + k ** 2) ** 0.5 > (
                                radius - 1.49):
    ...
    def commandCurve(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 11 and len(parts) != 2:
            ...
            if limit != -1:
                # Stop them doing silly things
                estimatedBlocks = (2 * ((x - x2) ** 2 + (y - y2) ** 2 + (z - z2) ** 2) ** 0.5 + 2 * ((x2 - x3) ** 2 + (
                                                                                                                      y2 - y3) ** 2 + (
                                                                                                                                      z2 - z3) ** 2) ** 0.5)
        ...
        if len(parts) < 7 and len(parts) != 4:
            ...
            if limit != -1:
                if int(((x * y) * (z)) / 3 > limit) or limit == 0:
                    self.client.sendServerMessage(
                        "Sorry, that area is too big for you to pyramid. (Limit is %s)" % limit)
                    return
            pointlist = []
                            ...
                            for j in range(y, y2 + 1):
                                for k in range(z, z2 + 1):
                                    if not self.client.AllowedToBuild(i, j, k) and not overriderank:
                                        return
                                    if fill == "true" or (i == x and j == y) or (i == x2 and j == y2) or (
                                    j == y2 and k == z2) or\
                                       (i == x2 and k == z2) or (j == y and k == z) or (i == x and k == z) or (
                                    i == x and k == z2) or\
                                       (j == y and k == z2) or (i == x2 and k == z) or (j == y2 and k == z) or (
                                    i == x and j == y2) or (i == x2 and j == y):
                                        world[i, j, k] = block
                                        self.client.queueTask(TASK_BLOCKSET, (i, j, k, block), world=world)
                                        self.client.sendBlock(i, j, k, block)
                                        yield
    ...
    def commandCsphere(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 7 and len(parts) != 4:
            ...
            if limit != -1:
                ...
                if ((radius * 2) ** 3 > limit) or limit == 0:
                    self.client.sendServerMessage(
                        "Sorry, that area is too big for you to csphere (Limit is %s)" % limit)
                    return
                # Draw all the blocks on, I guess
            # We use a generator so we can slowly release the blocks
            # We also keep world as a local so they can't change worlds and affect the new one
        ...
        if len(parts) < 9 and len(parts) != 3:
            ...
            if limit != -1:
                ...
                if (int(4 / 3 * cmath.pi * radius ** 2 * endradius) > limit) or limit == 0:
                    self.client.sendServerMessage(
                        "Sorry, that area is too big for you to ellipsoid (Limit is %s)" % limit)
                    return
                # Draw all the blocks on, I guess
            # We use a generator so we can slowly release the blocks
            # We also keep world as a local so they can't change worlds and affect the new one
                        ...
                        for j in range(-radius - 2, radius + 1):
                            for k in range(-radius - 2, radius + 1):
                                if (((i + var_x - x) ** 2 + (j + var_y - y) ** 2 + (k + var_z - z) ** 2) ** 0.5 + ((
                                                                                                                   i + var_x - x2) ** 2 + (
                                                                                                                                          j + var_y - y2) ** 2 + (
                                                                                                                                                                 k + var_z - z2) ** 2) ** 0.5) / 2 + 0.691 < radius:
                                    if not self.client.AllowedToBuild(x + i, y + j, z + k) and not overriderank:
                                        self.client.sendServerMessage("You do not have permision to build here.")
                                        return
                                    world[var_x + i, var_y + j, var_z + k] = block
                                    self.client.queueTask(TASK_BLOCKSET, (var_x + i, var_y + j, var_z + k, block),
                                        world=world)
    ...
    def commandPolytri(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 11 and len(parts) != 2:
            ...
            if limit != -1:
                # Stop them doing silly things
                if ((((x - x2) ** 2 + (y - y2) ** 2 + (z - z2) ** 2) ** 0.5 * ((x - x3) ** 2 + (y - y3) ** 2 + (
                                                                                                               z - z3) ** 2) ** 0.5) > limit) or limit == 0:
                    self.client.sendServerMessage(
                        "Sorry, that area is too big for you to polytri (Limit is %s)" % limit)
                    return
            ...
            if limit != -1:
                ...
                if (height * 4 > limit) or limit == 0:
                    self.client.sendSplitServerMessage(
                        "Sorry, that area is too big for you to make stairs (Limit is %s)" % limit)
                    return
                # Draw all the blocks on, I guess
            # We use a generator so we can slowly release the blocks
            # We also keep world as a local so they can't change worlds and affect the new one
        ...
        if len(parts) < 7 and len(parts) != 4:
            ...
            def generate_changes():
                for i in range(-radius - 1, radius):
                    for j in range(-radius - 1, radius):
                        for k in range(-radius - 1, radius):
                            if ((i ** 2 + j ** 2 + k ** 2) ** 0.5 + .604 < radius) and (
                            (i ** 2 + j ** 2 + k ** 2) ** 0.5 + .604 > radius - 1.208):
                                if not ((i != 0 and normalAxis == "x") or (j != 0 and normalAxis == "y") or (
                                k != 0 and normalAxis == "z")):
                                    if not self.client.AllowedToBuild(x + i, y + j, z + k) and not overriderank:
                                        self.client.sendServerMessage("You do not have permission to build here.")
    ...
    def commandHCylinder(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 8 and len(parts) != 5:
            ...
            limit = self.client.getBlbLimit()
            if limit != -1:
                if (int((cmath.pi * (radius ** 2) * height) - (
                cmath.pi * ((radius - 1) ** 2) * height)) > limit) or limit == 0:
                    ...
                    for i in range(-radius - 1, radius):
                        for j in range(-radius - 1, radius):
                            for k in range(-radius - 1, radius):
                                ...
                                (i ** 2 + j ** 2 + k ** 2) ** 0.5 + .604 > (radius - 1.208)):
                                    if not (
                                    (i != 0 and axis == "x") or (j != 0 and axis == "y") or (k != 0 and axis == "z")):
                                        if not self.client.AllowedToBuild(x + i, y + j, z + k) and not overriderank:
                                            self.client.sendServerMessage("You do not have permission to build here.")
                                            return
                                        try:
        ...
        if len(parts) < 8 and len(parts) != 5:
            ...
            if limit != -1:
                if (cmath.pi * (radius ** 2) * height > limit) or limit == 0:
                    self.client.sendSplitServerMessage(
                        "Sorry, that area is too big for you to make a cylinder. (Limit is %s)" % limit)
                    return
                # Draw all the blocks on, I guess
            # We use a generator so we can slowly release the blocks
            # We also keep world as a local so they can't change worlds and affect the new one
