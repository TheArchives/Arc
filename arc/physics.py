...
class Physics(Thread):
    ...
    def run(self):
        while self.running:
            if self.blockstore.physics:
                self.logger.debug("Starting physics run for '%s'. (a%i, f%i, s%i, gg%i, gd%i, sf%i)" % (self.blockstore.blocks_path, len(self.air_queue), len(self.fluid_queue), len(self.sponge_queue), len(self.grass_grow_queue), len(self.grass_die_queue), len(self.sand_queue)))
                ...
                self.logger.debug("Ended physics run for '%s' (c%i, a%i, f%i, s%i, gg%i, gd%i, sf%i)." % (self.blockstore.blocks_path, len(changes), len(self.air_queue), len(self.fluid_queue), len(self.sponge_queue), len(self.grass_grow_queue), len(self.grass_die_queue), len(self.sand_queue)))
            else:
                if self.was_physics:
                    self.init_queues()
    ...
    def run_iteration(self):
        ...
        if not self.blockstore.unflooding:
            try:
                while fluid_done < self.FLUID_LIMIT:
                    offset = self.fluid_queue.pop()
