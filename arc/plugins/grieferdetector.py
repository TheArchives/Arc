            ...
            if ord(world.blockstore.raw_blocks[world.blockstore.get_offset(x, y, z)]) != 3:
                ...
                def griefcheck():
                    if self.var_blockchcount >= self.client.factory.grief_blocks:
                        ...
                        self.client.factory.queue.put((self.client, TASK_STAFFMESSAGE, (0, COLOUR_DARKGREEN, "Console", ("#%s%s: %s%s" % (COLOUR_DARKGREEN, 'Console ALERT', COLOUR_DARKRED, ("World: "+worldname+" | User: "+username)))), False))
